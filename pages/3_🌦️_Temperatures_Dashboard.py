...
min_temp_city = (temps_df[temps_df["AvgTemperatureFahrenheit"] == min_temp]["City"].apply(lambda x : x.split(' ')[0] if x else None))  # TODO
min_temp_date = temps_df[temps_df["AvgTemperatureFahrenheit"] == min_temp]["Date"]  

max_temp_city = temps_df[temps_df["AvgTemperatureFahrenheit"] == max_temp]["City"]  
max_temp_date = temps_df[temps_df["AvgTemperatureFahrenheit"] == max_temp]["Date"] 
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        city_df = temps_df.query('City ==@city') #temps_df[temps_df["City"] == city]            # TODO: get a dataframe with the rows of the selected city
        city_df_period = city_df.query('Date>@start_date & Date<@end_date') #temps_df[(start_date <= temps_df["Date"]) | (temps_df["Date"] >= end_date)]    # TODO: get a dataframe with the rows of the selected city and the selected period of time using the Date column and any of the <, >, <=, >= operators to compare with start_date and end_date
        plt.plot(city_df_period["Date"], city_df_period["AvgTemperatureCelsius"], label = f"Temperature of {city}")                # TODO plot each city line and use the label parameter to set the legend name for each city

    plt.title(f"Temperature of {selected_cities}")   # TODO
    plt.xlabel("Timestamp")  # TODO
    ...
    for city in selected_cities:
        ...
        city_df_period = city_df.query('Date>@start_date & Date<@end_date')     # TODO: get a dataframe with the rows of the selected city and the selected period of time using the Date column and any of the <, >, <=, >= operators to compare with start_date and end_date
        plt.hist(city_df['AvgTemperatureCelsius'], bins=20, edgecolor='white', alpha=0.6, label=f'AVG °C in {city}')                 # TODO: plot each city histogram in the same plot and use the label parameter to set the legend name for each city 
